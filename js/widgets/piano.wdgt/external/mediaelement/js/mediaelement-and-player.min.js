...
mejs.HtmlMediaElementShim = {
  ...
  determinePlayback: function(a, b, c, d, e) {
    ...
    if (
      ...
      for (f = 0; f < g.length; f++)
        if (
          /* Remko added this null check, because we were seeing it a lot. Not sure what it means that this is null. */
          g[f].type != null &&
          (g[f].type == "video/m3u8" ||
            a.canPlayType(g[f].type).replace(/no/, "") !== "" ||
            a
              .canPlayType(g[f].type.replace(/mp3/, "mpeg"))
              .replace(/no/, "") !== "" ||
            (mejs.MediaFeatures.isFirefox == false &&
              a
                .canPlayType(g[f].type.replace(/m4a/, "mp4"))
                .replace(/no/, "") !== ""))
        ) {
          h.method = "native";
          h.url = g[f].url;
  ...
  getTypeFromFile: function(a) {
    ...
    return (
      (/(mp4|m4v|ogg|ogv|m3u8|webm|webmv|flv|wmv|mpeg|mov)/gi.test(a)
        ? "video"
        : "audio") +
      "/" +
...
(function(f) {
  ...
  mejs.MediaElementPlayer.prototype = {
    ...
    setPlayerSize: function(a, b) {
      ...
      if (
        ...
        var c = this.isVideo
            ? this.media.videoWidth && this.media.videoWidth > 0
              ...
              : this.media.getAttribute("width") !== null
                ? this.media.getAttribute("width")
          ...
          e = this.isVideo
            ? this.media.videoHeight && this.media.videoHeight > 0
              ...
              : this.media.getAttribute("height") !== null
                ? this.media.getAttribute("height")
      ...
      g.css(
        "margin-top",
        "-" + (g.height() / 2 - this.controls.height() / 2).toString() + "px"
      );
    },
    ...
    onkeydown: function(a, b, c) {
      if (a.hasFocus && a.options.enableKeyboard)
        for (var e = 0, d = a.options.keyActions.length; e < d; e++)
          for (
            var g = a.options.keyActions[e], k = 0, j = g.keys.length;
            k < j;
            k++
          )
  ...
  f.extend(MediaElementPlayer.prototype, {
    ...
    buildfullscreen: function(a, b, c, e) {
      if (a.isVideo) {
        ...
        mejs.MediaFeatures.hasTrueNativeFullScreen &&
          a.globalBind(mejs.MediaFeatures.fullScreenEventName, function() {
            if (a.isFullScreen)
    ...
    enterFullScreen: function() {
      ...
      if (
        ...
        a.container
          ...
          .css("font-size", screen.width / a.width * 1 * 100 + "%");
    ...
    buildspeed: function(a, b, c, e) {
      if (this.media.pluginType == "native") {
        c =
          ...
          'x</button><div class="mejs-speed-selector"><ul>';
        var d;
        f.inArray(this.options.defaultSpeed, this.options.speeds) === -1 &&
          this.options.speeds.push(this.options.defaultSpeed);
        this.options.speeds.sort(function(g, k) {
          return parseFloat(k) - parseFloat(g);
        });
        for (d = 0; d < this.options.speeds.length; d++) {
          c +=
            '<li><input type="radio" name="speed" value="' +
            this.options.speeds[d] +
            '" id="' +
            this.options.speeds[d] +
            '" ';
        ...
        b.height(
          this.speedButton.find(".mejs-speed-selector ul").outerHeight(true) +
        ...
        success: function(e) {
          c.entries =
            typeof e == "string" && /<tt\s+xml/gi.exec(e)
              ...
              : mejs.TrackFormatParser.webvtt.parse(e);
          c.isLoaded = true;
        ...
        if (b !== null && b.isLoaded)
          for (a = 0; a < b.entries.times.length; a++)
            if (
              ...
              this.captionsText
                .html(b.entries.text[a])
                .attr(
                  "class",
                  "mejs-captions-text " + (b.entries.times[a].identifier || "")
                );
